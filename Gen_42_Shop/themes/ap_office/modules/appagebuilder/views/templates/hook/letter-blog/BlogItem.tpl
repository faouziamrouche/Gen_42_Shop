    ...
    <div class="left-block">
        <div class="blog-image-container">
            ...
            {if isset($formAtts.show_title) && $formAtts.show_title}
	        	<h5 class="blog-title" itemprop="name"><a href="{$blog.link}{*full url can not escape*}" title="{$blog.title|escape:'html':'UTF-8'}">{$blog.title|strip_tags:'UTF-8'|truncate:30:'...'}</a></h5>
	        {/if}
        </div>
    </div>
    <div class="right-block">
    	<div class="box-blog">
			...
			{if isset($formAtts.bleoblogs_saut) && $formAtts.bleoblogs_saut}
				<span class="author">
					<span class="icon-author"> {l s='Post by' d='Shop.Theme.Global'}:</span> {$blog.author|escape:'html':'UTF-8'}
				</span>
			{/if}		
			...
			{if isset($formAtts.bleoblogs_scoun) && $formAtts.bleoblogs_scoun}
				<span class="nbcomment">
					<span class="icon-comment"><i class="material-icons">&#xE0CB;</i>{l s='Comment' d='Shop.Theme.Global'}: </span> {$blog.comment_count|intval}
				</span>
			{/if}
	        ...
	        {if isset($formAtts.bleoblogs_scre) && $formAtts.bleoblogs_scre}
				<span class="created">
					<time class="date" datetime="{strtotime($blog.date_add)|date_format:"%Y"}{*convert to date time*}">
						...
						{l s=strtotime($blog.date_add)|date_format:"%e" d='Shop.Theme.Global'}-<!-- day of month -->
						{l s=strtotime($blog.date_add)|date_format:"%m" d='Shop.Theme.Global'}-<!-- month-->
						{l s=strtotime($blog.date_add)|date_format:"%Y" d='Shop.Theme.Global'}<!-- year -->
					</time>
					{if $formAtts.class|strstr:'add-read-more'}
			            <a href="{$blog.link|escape:'html':'UTF-8'}" title="{$blog.title|escape:'html':'UTF-8'}" class="read-blog">{l s='View more' d='Shop.Theme.Global'}</a>
			        {/if}
				</span>
			{/if}
