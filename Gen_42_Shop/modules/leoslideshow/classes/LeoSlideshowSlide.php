...
//		}
    ...
    public static function freeCountSlide($id_lang = null, $id_shop = null)
    {
        if (!$id_shop) {
        ...
        $sql = 'SELECT * FROM '._DB_PREFIX_.'leoslideshow_slides';
        $sql .= ' WHERE id_group=' . (int)Tools::getValue('id_group');
        $slide = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($sql);
        $number_slide = count($slide);
        return $number_slide;
    }
}
